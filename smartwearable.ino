...
const char* serverName = "http://192.168.227.164:5000/post";
hw_timer_t * posttimer = NULL;
volatile int postflag = 0;
...
int16_t tremor_ax[ACC_BUF_LEN/3], tremor_ay[ACC_BUF_LEN/3], tremor_az[ACC_BUF_LEN/3];
...
//int loadingBar_cursor = 0;
//int loadingBar_end = 120;
/* 
  Display FSM
  0xF0 : Show Time
...
void IRAM_ATTR onposttmr() {
  postflag  = 1;
  Serial.println("hi");

}

void post_values(void * parameter){
...
void record_tremor(void * parameter){

  while(1){
  memmove(tremor_acc_buf, tremor_acc_buf + 3, (ACC_BUF_LEN - 3) * sizeof(int16_t));
  adxl.readAccel(tremor_xyz);
  //Serial.println(xyz[0]);
  tremor_acc_buf[ACC_BUF_LEN - 3] = (int16_t) tremor_xyz[0];
  tremor_acc_buf[ACC_BUF_LEN - 2] = (int16_t) tremor_xyz[1];
  tremor_acc_buf[ACC_BUF_LEN - 1] = (int16_t) tremor_xyz[2];
  ...
  for (uint16_t i = 0; i < ACC_BUF_LEN; i=i+3) {
        tremor_ax[j] = tremor_acc_buf[i];
        tremor_ay[j] = tremor_acc_buf[i+1];
        tremor_az[j] = tremor_acc_buf[i+2];
        j++;
    }

  for(int i = 0;i<10;i++)
  {
    Serial.print(tremor_ax[i]);
    Serial.print(tremor_ay[i]);
    Serial.println(tremor_az[i]);        
  }
...
//        vTaskDelay(500);
  ...
  if(displayState == 0xF1)
    {  
      beatAvg_new = beatAvg_10;
      display.clearDisplay();  
      //display.display();
      display.setCursor(1,1);
      display.println("  Average     BPM");
      display.print("    ");
      display.println(beatAvg_10);
      display.display();
      vTaskDelay(xDelay);
      displayState = 0xF0;
    }
...
// send beatAvg_10,last_fall_tm,accelerometer_data every 10 sec
...
void setup()
{
  ...
  posttimer = timerBegin(0, 80, true);
  timerAttachInterrupt(posttimer, &onposttmr, true);
  timerAlarmWrite(posttimer, 10000000, true);
  timerAlarmEnable(posttimer);
